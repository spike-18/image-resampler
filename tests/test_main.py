    ...
    for method in ["nn", "bl", "pw", "l2"]:
        assert (
            main_mod.main.callback(
                DummyFile(), method, 2, save=False, verbose=False
            )
            is None
        )
    # Test invalid method
...
def test_main_save_and_verbose(monkeypatch, tmp_path) -> None:
    ...
    called = {}
    monkeypatch.setattr(
        main_mod, "load_image", lambda *_a, **_k: __import__("numpy").zeros((8, 8))
    )
    monkeypatch.setattr(
        main_mod, "save_image", lambda *_a, **_k: called.setdefault("saved", True)
    )
    monkeypatch.setattr(
        main_mod, "nearest_neighbor", lambda *_a, **_k: __import__("numpy").zeros((8, 8))
    )
