...
def benchmark(input, scale):
    ...
    low_res = resize(original, down_shape, anti_aliasing=True, preserve_range=True).astype(original.dtype)
    ...
    for name, func in methods.items():
        ...
        if upscaled.shape != original.shape:
            upscaled = resize(upscaled, original.shape, anti_aliasing=True, preserve_range=True).astype(original.dtype)
